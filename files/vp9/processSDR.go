...
func process320p(inputFilePath string, outputFilePath string, log logger.Log) error {
	log.Debug("pipeline checkpoint", "file", inputFilePath, "encoder", "vp9", "media-type", "video-sdr", "resolution", "180x320", "status", "starting processing")
	var cmd *exec.Cmd
	var err error
	...
	cmd = exec.Command("ffmpeg", "-i", inputFilePath, "-vf", "scale=180x320", "-b:v", "150k", "-minrate", "75k", "-maxrate", "218k", "-tile-columns", "0", "-g", "240", "-threads", "2", "-quality", "good", "-crf", "37", "-c:v", "libvpx-vp9", "-an", "-pass", "1", "-speed", "4", outputFilePath)
	_, err = cmd.Output()
	if err != nil {
		log.Error("error in processing video", "file", inputFilePath, "encoder", "vp9", "resolution", "180x320", "pass", "1", "error", err.Error())
		return err
	} else {
		log.Debug("pipeline checkpoint", "file", inputFilePath, "encoder", "vp9", "media-type", "video-sdr", "resolution", "180x320", "pass", "1", "status", "finished")
	}
	cmd = exec.Command("ffmpeg", "-y", "-i", inputFilePath, "-vf", "scale=180x320", "-b:v", "150k", "-minrate", "75k", "-maxrate", "218k", "-tile-columns", "0", "-g", "240", "-threads", "2", "-quality", "good", "-crf", "37", "-c:v", "libvpx-vp9", "-an", "-pass", "2", "-speed", "1", outputFilePath)
	_, err = cmd.Output()
	if err != nil {
		log.Error("error in processing video", "file", inputFilePath, "encoder", "vp9", "resolution", "180x320", "pass", "2", "error", err.Error())
		return err
	} else {
		log.Debug("pipeline checkpoint", "file", inputFilePath, "encoder", "vp9", "media-type", "video-sdr", "resolution", "180x320", "pass", "2", "status", "finished")
	}
	return nil
}

func process640p(inputFilePath string, outputFilePath string, log logger.Log) error {
	log.Debug("pipeline checkpoint", "file", inputFilePath, "encoder", "vp9", "media-type", "video-sdr", "resolution", "360x640", "status", "starting processing")
	var cmd *exec.Cmd
	var err error
